******************************************************************
* Simulador do PBF na PNAD 2011  *
* Rodrigo Quintana
* Dez/2012
******************************************************************
*Things to change for every survey

/* 
1. BF top (306 in 2011)
2. Missing gap and % it represents (26% in 2011)
	- Need to know estimate of hhs in Brazil that year
	- Number of families benefiting from BF that year 
3. Benefit rules
4. Line where bigger dif start
5. % of people who received fixed benefit (85% in 2011)
6. Wage deflector
*/
...
* Individuals
		...
		* Identification and clean up
			...
			rename uf state
			label define state_label 11 "Rond™nia" 12 "Acre" 13 "Amazonas" 14	"Roraima" /// 
					              15 "Par‡" 16	"Amap‡" 17 "Tocantins" 21 "Maranh‹o" ///
					              22 "Piau’" 23	"Cear‡" 24	"Rio Grande do Norte" ///
					              25 "Para’ba" 26 "Pernambuco" 27 "Alagoas" 28	"Sergipe" ///
					              29 "Bahia" 31	"Minas Gerais" 32 "Esp’rito Santo" ///
					              33 "Rio de Janeiro" 35 "S‹o Paulo" 41	"Paran‡" ///
					              42 "Santa Catarina" 43 "Rio Grande do Sul" ///
					              50 "Mato Grosso do Sul" 51 "Mato Grosso" 52 "Goi‡s" ///
					              53 "Distrito Federal"
...
******************************************************************
* PNAD: (19,993 pbfpnad/106,449 hhs pnad) * 57,324,167 actual hhs 2010 =~10,518,317 famílias; R$ 1 227 247 225
* SAGI/MDS, setembro/2011: 13,179,472 famílias; R$ 1 573 687 473
* Faltam: 2,661,155 famílias; 25.30% do total de identificadas
...
********************************************************************************
		...
		/* Output
			log using "/Users/rodrigoquintana/dropbox/PED 250Y - SYPA/PNAD/Data/Rawdata_stata/pnad_hot_deck_pbf", replace
			set more off
			* Calculos para a tabela 1
				forvalues i=0/1 { 
					xtile probolsa1 if pbfpnad==`i' [w=pesohh], n(100) gen(xtiles) 
					tabstat probolsa1 [w=pesohh] if pbfpnad==`i', by(xtiles) save
					tabstatmat probolsa1_`i'
					drop xtiles
				}
			* Tabela 1: probabilidade predita de PBF para identificados e não identificados
				di " "
				di "Tabela 1: probabilidade predita de PBF para identificados e não identificados"
				qui matrix define probolsa = (100*probolsa_0, 100*probolsa_1)
				qui matrix colnames probolsa = naopbf simpbf
				matrix list probolsa, f(%5.1f)
				di " "
			* Cálculos para a tabela 2
				foreach peso in pesohh peso { 
					qui tab catpbf [w=`peso'], matcell(cat_`peso')
					qui matrix define cat_`peso' = ( cat_`peso' \ 100*cat_`peso' / r(N) )
				} 
				matrix define catpbf = (cat_pesohh, cat_peso)
				matrix rownames catpbf = npnad nsimu nnaop ppnad psimu pnaop
				matrix colnames catpbf = doms pess
			* Tabela 2: distribuição absoluta e relativa das categorias
				di " " 
				di "Tabela 2: distribuição absoluta e relativa das categorias"
				matrix list catpbf, f(%12.2f)
				di " " 
			* Tabela 3: renda e probabilidades por categorias
				di " " 
				di "Tabela 3: renda e probabilidades por categorias"
				table catpbf [w=pesohh], c(mean probo mean incomepbf mean incomeliqpc mean incomepbfpc) f(%10.4f)
			qui log close*/
		* Save
			save "/Users/rodrigoquintana/dropbox/PED 250Y - SYPA/PNAD/Data/Rawdata_stata/pnad2011_clean", replace		
******************************************************************		
